...
<div class="rsform">
    ...
    <div class="alert alert-success"><?php foreach ($arResult['MESSAGES']['SUCCESS'] as $msg): ?><?=$msg?><br><?php endforeach; ?>
	   <?
	    if(file_exists($_SERVER['DOCUMENT_ROOT'].'/include/forms/product_ask/message_after.php')){
	        include $_SERVER['DOCUMENT_ROOT'].'/include/forms/product_ask/message_after.php';
	    }
	    ?>
    </div>
    <script>$(document).trigger("rs_forms.success");</script>
	<?php } else {
    	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/include/forms/product_ask/message_before.php')){
			echo '<div class="alert alert-info">';
    		include $_SERVER['DOCUMENT_ROOT'].'/include/forms/product_ask/message_before.php';
    		echo '</div>';
	    }
    	?>
    <form action="<?=$arResult['REQUEST_URI']?>" method="POST" class="form rsfrom__form" id="<?=$arResult['FORM_NAME']?>" onsubmit="gtag('event', 'clickdopphone', { 'event_category': 'bystry_zakaz', 'event_action': 'otpravka_form' });">
        <?=bitrix_sessid_post()?>
              ...
              <div class="captcha_wrap">
                 ...
                 <div class="row">
                    <div class="rsform__captcha-input col col-md-6 col-xs-12">
                       ...
                       <input class="form-control req-input form-item" type="text" name="captcha_word" id="<?=$arResul['FORM_NAME']?>_captcha_word" size="30" maxlength="50" value="" required autocomplete="off">
                    </div>
