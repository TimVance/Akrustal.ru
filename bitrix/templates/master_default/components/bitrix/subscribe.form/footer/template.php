...
<div class="b-footer-subscribe">
    ...
    <form class="b-footer-subscribe__form" action="<?=$arResult['FORM_ACTION']?>" onsubmit="gtag('event', 'cel', { 'event_category': 'footer_subscribe', 'event_action': 'otpravka_form' });">
    ...
    try {
        if (VK && VK.Observer && VK.Observer.subscribe) {
            VK.Observer.subscribe('widgets.like.liked', function(event) {
                console.log('ga track vk like');
                ga('send', 'social', {
                    'socialNetwork': 'vk',
                    'socialAction': 'like'
                });
            });
            VK.Observer.subscribe('widgets.like.unliked', function(event) {
                console.log('ga track vk unlike');
                ga('send', 'social', {
                    'socialNetwork': 'vk',
                    'socialAction': 'unlike'
                });
        ...
        console.log('VK script instalation failed', e);
    };
</script>