(function(window){
	...
	window.JCCatalogElement.prototype = {
      ...
      this.quantityChange();
		...
		sendToBasket: function()
		{
			...
			// Орлов А. добавление в корзину Битрикс
			this.initBasketUrl();
		...
		basketResult: function(arResult)
		{
			...
			else
			{
				...
				if (arResult.STATUS === 'OK')
				{
					...
					popupContent = '<div class="popup-window-data"><div class="popup-window-data-img"><img src="'
						+ productPict + '" height="200" style="max-height:200px"></div><p class="popup-window-data-text">' + this.product.name + '</p></div>';
    ...
    checkFavorite: function(){
      //e.preventDefault();
      var obLinkText = BX.findChild(this.obFavoriteBtn, {'className': 'favorite__text'}, true),
          obFavoriteCount = BX.findChild(this.obFavoriteBtn, {'className': 'favorite__cnt'}, true);

      if (BX.util.in_array(this.product.id, RS.Favorite.getItems())) {
        if (!!obLinkText) {
          obLinkText.innerHTML = BX.message('RS_FAVORITE_DEL');
        }
      } else {
        if (!!obLinkText) {
          obLinkText.innerHTML = BX.message('RS_FAVORITE_ADD');
        }
      }
    },
	}
