...
$arParams['LIST_SHOW_SIDEBAR'] = (isset($arParams['LIST_SHOW_SIDEBAR']) && $arParams['LIST_SHOW_SIDEBAR'] == 'Y' ? 'Y' : 'N');
...
$isFilter = ($arParams['USE_FILTER'] == 'Y');
	...
	elseif ($obCache->StartDataCache())
	{
		...
		if (Loader::includeModule("iblock"))
		{
            ...
            if (is_array($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']]) && count($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']]) > 0) {
                ...
                if ($arFeatureFilter['USER_TYPE_ID'] == 'hlblock') {
                    ...
                    $arFeatureFilter['VALUES'] = $hlDataClass::getList(array(
                        // 'select' => array('ID'),
                        'filter' => array('=ID' => array_values($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']]))
                    ))->fetchAll();
                }


                foreach ($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']] as $iFeatureFilterkey => $sFeatureFilterId) {
                    foreach ($arFeatureFilter['VALUES'] as $arFieatureFilterVal) {
                        if ($arFieatureFilterVal['ID'] == $sFeatureFilterId) {
                            ...
                            if (isset($arResult['VARIABLES']['SMART_FILTER_PATH'])) {
                                $tmp = str_replace(explode('/', $arFieatureFilterVal['UF_LINK']), '', $arResult['VARIABLES']['SMART_FILTER_PATH']);
                                if (str_replace('/', '', $tmp) == '') {
                                    $arFieatureFilterVal['CHECKED'] = true;
                                }
                            }



                            $arFieatureFilterVal['UF_LINK'] = $arCurSection['SECTION_PAGE_URL'].$arFieatureFilterVal['UF_LINK'];
                            //if ($arResult['VARIABLES']['SMART_FILTER_PATH'])

                            $arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']][$iFeatureFilterkey] = $arFieatureFilterVal;
                            break;
                        }
                    }
...
*/?>
        ...
        <?php if ($arParams['USE_FILTER'] == 'Y'): ?>
            <div class="catalog__filter-top filter_top">
                ...
                <?php if (is_array($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']]) && count($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']]) > 0): ?>
                    <div class="filter_top__box">
                        <?php foreach ($arCurSection[$arParams['FEATURE_FILTER_USER_FIELDS']] as $arFeatureFilter): ?>
                            <a class="filter_top__item<?=$arFeatureFilter['CHECKED'] ? ' checked': '';?>" href="<?=$arFeatureFilter['UF_LINK'];?>">
                                <span class="filter_top__btn"><?=$arFeatureFilter['UF_NAME'];?></span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
...
<?php
if ($arParams['LIST_SHOW_SIDEBAR'] == 'N') {
    $APPLICATION->SetPageProperty('hide_sidebar', 'Y');
    $APPLICATION->SetPageProperty('wide_page', 'N');
}
