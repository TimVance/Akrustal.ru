...
$(document).ready(function (){
	...
	$('a[data-toggle="tab"]:not(.linked)').on('shown.bs.tab', function(e){
		...
		if ($(this).attr("href").split('#')[1] == "custom_tab") loadMap();
...
function loadMap() {
	let path = '/include/download_map.php?7';
	$.ajax({
		url: path,
	}).done(function(result) {
		$(".download-map").html(result);
		//$("body").append('<script src="/bitrix/templates/aspro_max/components/ao/sale.geography/detail_page/script.js?v=2">');
		DrawMap();
	});
}
...
function DrawMap() {
	...
	//$(document).ready(function () {
		...
		//if ($(".download-map").length == 0) return false;
		//.$(".download-map").append("test3");
		/* Яндекс.Карта */
		...
		$sale_geography_accordion.on('shown.bs.collapse', function () {
			var $active = $sale_geography.find('.panel-collapse.in').closest('.panel');
		...
		function refreshPlacemarks(all) {
			...
			$sale_geography.find('.b-sale-geography__city'+(!all?'.panel-collapse.in':'')+' .shop-data').each(function(){
				...
				//$(".viewed-wrapper").append(Placemark[Obj]);
				//console.log($myClusterer);
			...
			else
			{
				...
				{
					$sale_geography.find('.b-sale-geography__city.panel-collapse.in').each(function () {
			...
			if (Object.keys(Placemark).length !== 0)
			{
				...
				$sale_geography_map.animate({'height':'600px'}, 'fast', 'linear', function() {
					$sale_geography_map.show();
