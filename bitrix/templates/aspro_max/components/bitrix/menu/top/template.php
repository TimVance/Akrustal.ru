					...
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<div class="dropdown-menu <?=$bRightSide ? 'with_right_block' : ''?> <?=$arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']?>">
												...
												<?else: // type 1?>
													...
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?>">
														<?if($bHasPicture && $bWideMenu):
															...
															if(is_array($arImg)):?>
																<div class="menu_img <?=$bIcon ? 'icon' : ''?>">
																	<a href="<?=$arSubItem["LINK"]?>" class="noborder img_link colored_theme_svg">
																		<?if(strpos($arImg["src"], ".svg") !== false):?>
																			<?=CMax::showIconSvg("cat_icons light-ignore", $arImg["src"]);?>
																		<?else:?>
																			<img class="lazy" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImg["src"]);?>" data-src="<?=$arImg["src"]?>" alt="<?=$arSubItem["TEXT"]?>" title="<?=$arSubItem["TEXT"]?>" />
															...
															<?endif;?>
														<?endif;?>
														<a href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name <?=$bWideMenu ? 'option-font-bold' : ''?>"><?=$arSubItem["TEXT"]?></span><?=($arSubItem["CHILD"] && $bShowChilds ? CMax::showIconSvg("right light-ignore", SITE_TEMPLATE_PATH.'/images/svg/trianglearrow_right.svg', '', '') : '')?></a>
														<?if($arSubItem["CHILD"] && $bShowChilds):?>
															...
															<ul class="dropdown-menu toggle_menu">
																...
																<?if($iCountChilds > $iVisibleItemsMenu && $bWideMenu):?>
																	<li><span class="colored_theme_hover_text more_items with_dropdown"><?=\Bitrix\Main\Localization\Loc::getMessage("S_MORE_ITEMS").' '.($iCountChilds-$iVisibleItemsMenu);?></span></li>
																<?endif;?>
															</ul>
														<?endif;?>
													</li>
