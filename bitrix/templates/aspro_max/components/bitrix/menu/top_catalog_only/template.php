					...
					<td class="menu-item <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($bWideMenu ? 'wide_menu' : '');?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<?$bRightSideShow = ($arItem["BANNERS"] || $arBrands) && $bWideMenu && $bRightSide;?>
												...
												<?else: // type 1?>
													...
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?>">
														<?if($bHasPicture && $bWideMenu):
															...
															<?endif;?>
														<?endif;?>
														<a href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name"><?=$arSubItem["TEXT"]?></span><?=($arSubItem["CHILD"] && $bShowChilds ? CMax::showIconSvg("right light-ignore", SITE_TEMPLATE_PATH.'/images/svg/trianglearrow_right.svg', '', '') : '')?></a>
