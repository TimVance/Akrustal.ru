...
<?else:?>
	...
	$arYears = array();//var_dump(array_merge($arItemFilter, array("SECTION_ID" => $arSection["ID"])));
	if($arItems)
	{
	...
	{
		$arElements = CMaxCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array_merge($arItemFilter, array("SECTION_ID" => $arItem["ID"])), false, false, array('ID', 'TAGS', 'IBLOCK_SECTION_ID'));
		if(!$arElements)
