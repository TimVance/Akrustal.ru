...
#\Akrustal\Lib\Utils::pr($arResult);

if ($normalCount > 0):
?>

<div class="basket_applied_discounts">
	<?
	# Орлов А. выводим скидки
	$USER_ID = $GLOBALS["USER"]->GetID();
	# накопительные  скидки
	$arUserDiscount = \CCatalogDiscountSave::GetDiscount(array('USER_ID' => $USER_ID));
	# скидки корзины
	$arUserDiscountIDs = [];
	if(count($arUserDiscount) || count($arResult['APPLIED_DISCOUNT_LIST'])) {
		?>
		<span class="basket_applied_discounts__header">Примененные скидки</span>:
		<?
		$str_discounts = '';
		foreach($arResult['APPLIED_DISCOUNT_LIST'] as $arDiscount) {
			$arUserDiscountIDs[] = $arDiscount['ID'];
			$str_discounts .= $arDiscount['NAME'].', ';
		}
		if(count($arUserDiscount)){
			foreach($arUserDiscount as $arDiscount) {
				$str_discounts .= $arDiscount['NAME'].' (<span class="basket_applied_discounts__header">'.$arDiscount['VALUE'].'%</span>), ';
			}
		}
		echo rtrim($str_discounts,', ');
	}
	?>
</div>

<?
	# ищем ближайшую сверху по сумме скидку
	$order_sum = $arResult['allSum'];
	$arDiscountsIDS = array();
	...
	while ($groupDiscount = $groupDiscountIterator->fetch()) {
		...
		if ($groupDiscount['DISCOUNT_ID'] > 0) {
			$arDiscountsIDS[$groupDiscount['DISCOUNT_ID']] = $groupDiscount['DISCOUNT_ID'];
		}
	}

	if ($arDiscountsIDS) {
		$discountIterator = Bitrix\Sale\Internals\DiscountTable::getList(array(
			                                                                 'select' => array("ID", "NAME", "PRIORITY", "SORT", "LAST_DISCOUNT", "UNPACK", "APPLICATION", "CONDITIONS_LIST", "ACTIONS_LIST","SHORT_DESCRIPTION_STRUCTURE"),
			                                                                 'filter' => array(
				                                                                 '@ID' => $arDiscountsIDS,
				                                                                 '=USE_COUPONS' => 'N'
			                                                                 ),
		...
		while ($discount = $discountIterator->fetch()) {
			...
			if(!isset($discount['CONDITIONS_LIST']['CHILDREN'][0]['CLASS_ID']) || $discount['CONDITIONS_LIST']['CHILDREN'][0]['CLASS_ID'] != 'CondBsktAmtBaseGroup'){
				continue;
			}
			if(!isset($discount['CONDITIONS_LIST']['CHILDREN'][0]['DATA']) || !isset($discount['CONDITIONS_LIST']['CHILDREN'][0]['DATA']['logic'])){
				continue;
			}
			if($discount['CONDITIONS_LIST']['CHILDREN'][0]['DATA']['logic'] != 'EqGr'){
				continue;
			}
			if(!isset($discount['CONDITIONS_LIST']['CHILDREN'][0]['DATA']['Value']) || floatval($discount['CONDITIONS_LIST']['CHILDREN'][0]['DATA']['Value']) == 0){
				continue;
			}
			$next_summ = floatval($discount['CONDITIONS_LIST']['CHILDREN'][0]['DATA']['Value']);
			if($order_sum >= $next_summ){
				continue;
			}

			$nextDiscount = $discount['NAME'];
			break;
		}

		if($nextDiscount != "" && $next_summ != 0){
			?>
			<div class="basket_next_discount">Наберите <span class="basket_next_discount_summ"><?=($next_summ-1)?></span> руб. и получите скидку : <?=$nextDiscount?></div>
			<?
		}
	}
?>
					...
					>
							...
							?>
								<td class="custom">
									...
									<div class="basket_quantity">
											<div class="basket_quantity_input"><input
														type="text"
														size="3"
						...
						?>
							<td class="control">
								...
								if ($bDeleteColumn):
									?>
									<a class="btn" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"
										onclick="return deleteProductRow(this)"> X </a>
									<br />
									<?
								endif;
								if ($bDelayColumn):
								?>
									<a class="btn" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a>
								<?
								endif;
								?>
							</td>
