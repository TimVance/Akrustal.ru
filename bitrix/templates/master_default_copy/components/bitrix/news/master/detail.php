...
if(isset($arParams['BACK_BUTTON_URL'])) {
	$arElemFilter = array(
		"IBLOCK_ID"     => $arParams['IBLOCK_ID'],
		"CODE"          => $arDetailParams['ELEMENT_CODE']
	);
	$arElemOrder = array('SORT' => 'asc');
	$arElemSel = array("IBLOCK_SECTION_ID");
	$rsElem = CIBlockElement::GetList($arElemOrder, $arElemFilter ,false, false, $arElemSel);
	while ($arElem = $rsElem->GetNext()) {
		$back_url .= '#'.$arElem['IBLOCK_SECTION_ID'];
	}
}
