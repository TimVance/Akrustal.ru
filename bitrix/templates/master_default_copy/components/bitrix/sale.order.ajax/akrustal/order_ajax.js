	...
	BX.Sale.OrderAjaxComponent = {
		...
		editTotalBlock: function()
		{
			...
			if (this.options.showDiscountPrice)
			{
				...
				/* Орлов А. выведем скидки */
				if(this.result.DISCOUNT_LIST.length)
				{
					for (i = 0; i < this.result.DISCOUNT_LIST.length; i++)
					{
						this.totalInfoBlockNode.appendChild(this.createTotalUnit(this.result.DISCOUNT_LIST[i], 0, {highlighted: true}));
		...
		totalBlockScrollCheck: function()
		{
			...
			// Орлов А. необходимо учесть при отсупе панели итогов высоту верхнего фиксированного меню
			var toppading = 0;
			var toppanel = $('.b-fly-head.js-fly-head.is-active');
			if(toppanel.length > 0){
				toppading = toppanel.outerHeight();
			}
			this.totalInfoBlockNode.style.marginTop = toppading + 'px';
