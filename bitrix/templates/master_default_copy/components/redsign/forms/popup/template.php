...
<div class="rsform">
    ...
    <form action="<?=$arResult['REQUEST_URI']?>" method="POST" class="form rsfrom__form" id="<?=$arResult['FORM_NAME']?>" onsubmit="gtag('event', 'clickdopphone', { 'event_category': '<?php echo $arParams['EVENT_TYPE'] ?>', 'event_action': 'otpravka_form' });">
        <?=bitrix_sessid_post()?>
            ...
            <div class="row">
                ...
                <div class="form-group col col-md-12 field-wrap">
                      ...
                      <?php elseif ($arField['USER_TYPE'] == 'Date'): ?>
                          <input type="date" id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" value="<?=$arField['CURRENT_VALUE']?>" class="form-control<?=$disabled?>" <?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>>
                      <?php elseif ($arField['USER_TYPE'] == 'DateTime'): ?>
                          <input type="datetime-local" id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" value="<?=$arField['CURRENT_VALUE']?>" class="form-control<?=$disabled?>" <?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>>
                      <?php else: ?>
                          <input <?php if(isset($arField['MASK'])) echo 'data-inputmask="\'mask\': \''.$arField['MASK'].'\'"'; ?> type="<?=$arField['INPUT_TYPE']?>"<?php if(!empty($arField['PATTERN'])) echo ' pattern="'.$arField['PATTERN'].'"'; ?> id="FIELD_<?=$arField['CODE']?>" name="FIELD_<?=$arField['CODE']?>" value="<?=$arField['CURRENT_VALUE']?>" class="form-control<?=$disabled?>"<?=$disabled?><?php if ($arField['IS_REQUIRED'] == 'Y') echo ' required'?>>
                      <?php endif; ?>
                </div>
